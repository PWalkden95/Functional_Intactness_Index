...
With `GN1_2007__VanBael 1 9 64` I don't know whether to drop it as the value has biological relevance.
...
PREDICTS_site$Road_Density_1km <- scale(PREDICTS_site$density_1km^(1/2))
PREDICTS_site$Road_Density_50km <- scale(PREDICTS_site$density_50km^(1/2))


source("https://highstat.com/Books/Book2/HighstatLibV10.R")
corvif(PREDICTS_site[,c("LUI", "logHPD", "Road_Density_1km","Road_Density_50km" ,"UN_subregion")])

### a lot of colinearity try removing UN-subregion
corvif(PREDICTS_site[,c("LUI", "logHPD", "Road_Density_1km","Road_Density_50km")])
...
``` {r phylogenetic similarity, eval = FALSE, results='hide', message=FALSE, warning=FALSE}
...
site_vcv <- readRDS("../Functional_Intactness_Index/Outputs/site_vcv.rds")
...
test_mod <- lmer(Unbias_Rao ~ LUI + logHPD + Road_Density_1km  + Road_Density_50km +
...
Therefore, that does not indicate to me at the moment that I need to account for phylogenetic similarity of sites within the modelling framework therefore I will continue on with GLMMs, however, I will continue to use the `phyr` package as it will allow me to fit models using INLA and bayesian techniques, which helps with some convergence issues.
...
```{r PGLMM modeling, eval=FALSE, message=FALSE, warning=FALSE}
####################################
...
Rao_Model_1b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + logHPD + Road_Density_1km + Road_Density_50km +
...
Rao_Model_2b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + logHPD + Road_Density_1km + Road_Density_50km +
                      LUI:logHPD + LUI:Road_Density_1km + LUI:Road_Density_50km +
...
Rao_Model_3b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + logHPD + Road_Density_1km + Road_Density_50km +
                      LUI:logHPD + LUI:Road_Density_1km + LUI:Road_Density_50km +
...
Rao_Model_4b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + logHPD + Road_Density_1km + Road_Density_50km +
                      LUI:logHPD + LUI:Road_Density_1km + LUI:Road_Density_50km +
...
Rao_Model_5b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + logHPD + Road_Density_1km + Road_Density_50km +
                      LUI:logHPD + LUI:Road_Density_1km + LUI:Road_Density_50km +
...
```{r fixed effect structure, eval = FALSE}

#### Remove each interaction to see if thsis improves the model at all. 

#LUI:RD50k

Rao_Model_6b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + logHPD + Road_Density_1km + Road_Density_50km +
...
Rao_Model_6b$DIC - Rao_Model_1b$DIC   #### Model DIC is reduced by 9 and so the reduced model carries forward

## RD50k fixed

Rao_Model_7b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + logHPD + Road_Density_1km +
                                       LUI:logHPD + LUI:Road_Density_1km +
                                       (1|SS) + (1|SSB), data = PREDICTS_site, bayes = TRUE)
...
Rao_Model_7b$DIC - Rao_Model_6b$DIC   ####  DIC icreases so the previous model is retained


#### LUI:RD1k

Rao_Model_8b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + logHPD + Road_Density_1km + Road_Density_50km +
                                       LUI:logHPD +
                                       (1|SS) + (1|SSB), data = PREDICTS_site, bayes = TRUE)
...
Rao_Model_8b$DIC - Rao_Model_6b$DIC   #### Model DIC is reduced by 16 and so the reduced model carries forward


####  RD50k

Rao_Model_9b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + logHPD + Road_Density_1km +
                                       LUI:logHPD +
                                       (1|SS) + (1|SSB), data = PREDICTS_site, bayes = TRUE)
...
Rao_Model_9b$DIC - Rao_Model_8b$DIC   #### Model DIC increases so previous model is retained


### RD1k 

Rao_Model_10b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + logHPD + Road_Density_50km +
                                       LUI:logHPD +
                                       (1|SS) + (1|SSB), data = PREDICTS_site, bayes = TRUE)
...
Rao_Model_10b$DIC - Rao_Model_8b$DIC   #### Model DIC increases so the previous model is retained
...
Rao_Model_11b$DIC - Rao_Model_8b$DIC   #### Model DIC is reduced by 8 and so the reduced model carries forward


#### RD50k


Rao_Model_12b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + logHPD + Road_Density_1km +
                                       (1|SS) + (1|SSB), data = PREDICTS_site, bayes = TRUE)


summary(Rao_Model_12b)

Rao_Model_12b$DIC - Rao_Model_11b$DIC   #### Model DIC is reduced by 1 but that is higher than the threshold of 2 so the previous model is retained

#### RD1k

Rao_Model_13b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + logHPD + Road_Density_50km +
                                       (1|SS) + (1|SSB), data = PREDICTS_site, bayes = TRUE)


summary(Rao_Model_13b)

Rao_Model_13b$DIC - Rao_Model_11b$DIC   #### Model DIC increases so the previous model is retained

### logHPD


Rao_Model_14b <- phyr::communityPGLMM(Unbias_Rao ~ LUI + Road_Density_1km + Road_Density_50km +
                                       (1|SS) + (1|SSB), data = PREDICTS_site, bayes = TRUE)


summary(Rao_Model_14b)

Rao_Model_14b$DIC - Rao_Model_11b$DIC   ### Model DIC is reduced by 1 but that is higher than the threshold of 2 so the previous model is retained

####

summary(Rao_Model_11b)

site_vcv_test <- site_vcv[which(rownames(site_vcv) %in% PREDICTS_site$SSBS),which(rownames(site_vcv) %in% PREDICTS_site$SSBS)]
### best to experiment lets have a look at what happens when adding the site vcv to the model

```

### **Final Model**

After going through model selection the best model seems to be the one with LUI, logHPD and RD1k and RD50k as Fixed effects and no interactions between LUI and any of the other pressures with the best model being `Rao_Model_11b`. 
```{r save model}
...
hist(car::logit(Overlap_data$hyper_overlap, adjust = 0.001, percents = FALSE))
...
Overlap_data$rt3env <- scale(Overlap_data$rt3env)
...
Overlap_data$sqrtS2RD1K <- scale(Overlap_data$sqrtS2RD1K)
Overlap_data$sqrtS2RD50K <- scale(Overlap_data$sqrtS2RD50K)
...
corvif(Overlap_data[,c("Contrast", "logdist", "rt3env","S2logHPD", "logHPDdiff", "sqrtS2RD1K", "sqrtS2RD50K", "RD1Kdiff", "RD50Kdiff")])

### some colinearity problems lets drop site2HPD

corvif(Overlap_data[,c("Contrast", "logdist", "rt3env", "logHPDdiff", "sqrtS2RD1K", "sqrtS2RD50K", "RD1Kdiff", "RD50Kdiff")])


write_rds(file = "Functional_Intactness_Index/Overlap_modelling_data.rds", Overlap_data)
## that improve colinearity to acceptable levels  so we can move onto modeling 
```

```{r Overlap Modeling, eval = FALSE}
...
Model_1 <- lmer(logitOver ~ Contrast + logdist + rt3env + logHPDdiff + 
                  sqrtS2RD1K + sqrtS2RD50K + RD1Kdiff + RD50Kdiff + 
                  Contrast:logdist + Contrast:rt3env +  Contrast:logHPDdiff +
                  Contrast:sqrtS2RD1K + Contrast:sqrtS2RD50K + Contrast:RD1Kdiff + Contrast:RD50Kdiff +
                 (1|SS),
...
Model_2 <- lmer(logitOver ~ Contrast + logdist + rt3env + logHPDdiff + 
                  sqrtS2RD1K + sqrtS2RD50K + RD1Kdiff + RD50Kdiff + 
                  Contrast:logdist + Contrast:rt3env +  Contrast:logHPDdiff +
                  Contrast:sqrtS2RD1K + Contrast:sqrtS2RD50K + Contrast:RD1Kdiff + Contrast:RD50Kdiff +
                  (1|SS) + (1 + logdist|SS),
...
Model_3 <- lmer(logitOver ~ Contrast + logdist + rt3env + logHPDdiff + 
                  sqrtS2RD1K + sqrtS2RD50K + RD1Kdiff + RD50Kdiff + 
                  Contrast:logdist + Contrast:rt3env +  Contrast:logHPDdiff +
                  Contrast:sqrtS2RD1K + Contrast:sqrtS2RD50K + Contrast:RD1Kdiff + Contrast:RD50Kdiff +
                  (1|SS) + (1 + rt3env|SS),
...
Model_4 <- lmer(logitOver ~ Contrast + logdist + rt3env  + logHPDdiff + 
                  sqrtS2RD1K + sqrtS2RD50K + RD1Kdiff + RD50Kdiff + 
                  Contrast:logdist + Contrast:rt3env +  Contrast:logHPDdiff +
                  Contrast:sqrtS2RD1K + Contrast:sqrtS2RD50K + Contrast:RD1Kdiff + Contrast:RD50Kdiff +
...
Model_5 <- lmer(logitOver ~ Contrast + logdist + rt3env + logHPDdiff + 
                  sqrtS2RD1K + sqrtS2RD50K + RD1Kdiff + RD50Kdiff + 
                  Contrast:logdist + Contrast:rt3env +  Contrast:logHPDdiff +
                  Contrast:sqrtS2RD1K + Contrast:sqrtS2RD50K + Contrast:RD1Kdiff + Contrast:RD50Kdiff +
...
Model_6 <- lmer(logitOver ~ Contrast + logdist + rt3env + logHPDdiff + 
                  sqrtS2RD1K + sqrtS2RD50K + RD1Kdiff + RD50Kdiff + 
                  Contrast:logdist + Contrast:rt3env +  Contrast:logHPDdiff +
                  Contrast:sqrtS2RD1K + Contrast:sqrtS2RD50K + Contrast:RD1Kdiff + Contrast:RD50Kdiff +
...
Model_7 <- lmer(logitOver ~ Contrast + logdist + rt3env  + logHPDdiff + 
                  sqrtS2RD1K + sqrtS2RD50K + RD1Kdiff + RD50Kdiff + 
                  Contrast:logdist + Contrast:rt3env + Contrast:logHPDdiff +
                  Contrast:sqrtS2RD1K + Contrast:sqrtS2RD50K + Contrast:RD1Kdiff + Contrast:RD50Kdiff +
                  (1|SS) + (1 + RD1Kdiff|SS),
...
Model_8 <- lmer(logitOver ~ Contrast + logdist + rt3env  + logHPDdiff + 
                  sqrtS2RD1K + sqrtS2RD50K + RD1Kdiff + RD50Kdiff + 
                  Contrast:logdist + Contrast:rt3env +  Contrast:logHPDdiff +
                  Contrast:sqrtS2RD1K + Contrast:sqrtS2RD50K + Contrast:RD1Kdiff + Contrast:RD50Kdiff +
                  (1|SS) + (1 + RD50Kdiff|SS),
...
Model 1 seems to have the most support so we will carry on without any random slopes. Next stage is to perform backwards stepwise model simplification based on the liklihood ratio distribution test on 1000 permuted datasets.


```{r permuted data, eval = FALSE}
